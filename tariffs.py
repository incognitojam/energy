...
class Tariff:
    ...
    def __post_init__(self):
        self._unit_rate_lookup = Tariff.compile_unit_rate_lookup(
            self.day_unit_rate, self.additional_rates
        )
    ...
    def plot(self, ax=None):
        ...
        ax.plot(
            data.index.total_seconds() / 60, data.values, label=self.name
    ...
    @staticmethod
    def compile_unit_rate_lookup(
        day_unit_rate: float, additional_rates: list[Rate]
    ) -> pd.Series:
        """Compiles a lookup table for unit rates over a full day based on given day rate and additional rates."""
